...
function check_env_vars() {
  if [[ -v POSTGRESQL_USER || -v POSTGRESQL_PASSWORD || -v POSTGRESQL_DATABASE ]]; then
    ...
    [ ${#POSTGRESQL_DATABASE_2} -le 63 ] || usage "Database name too long (maximum 63 characters)"
...
function create_users() {
  if [[ ",$postinitdb_actions," = *,simple_db,* ]]; then
    ...
    [ -z "$POSTGRESQL_DATABASE_2" ] || createdb --owner="$POSTGRESQL_USER" "$POSTGRESQL_DATABASE_2"
  fi
